...
export default function NewsletterForm({ variant = "footer", className = "" }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setResult({ type: "success", text: data.message || t("newsletter.success") });
        setEmail("");
      } else {
        setResult({ type: "error", text: data.error || t("newsletter.error") });
      }
    } catch {
      setResult({ type: "error", text: t("newsletter.errorNetwork") });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={`bg-gradient-to-br from-primary-50/60 to-white dark:from-gray-800 dark:to-gray-800 rounded-2xl border border-primary-100/80 dark:border-gray-700 p-6 sm:p-8 shadow-sm ${className}`}>
      <div className="max-w-lg mx-auto text-center">
        <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">{t("newsletter.title")}</h3>
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-4">{t("newsletter.privacy")}</p>
      </div>
    </div>
  );
}
