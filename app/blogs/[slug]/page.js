...
export async function generateMetadata({ params }) {
  ...
  const defaultTitle = t(messages, "blog.postDefaultTitle");
  if (!slug) return { title: defaultTitle };
  try {
    const base = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
    const res = await fetch(`${base}/api/blogs/slug/${slug}`, { next: { revalidate: 60 } });
    if (!res.ok) return { title: defaultTitle };
...
export default async function BlogPostPage({ params }) {
  ...
  const dateStr = blog.published_at
    ? new Date(blog.published_at).toLocaleDateString(dateLocale, { dateStyle: "long" })
    : new Date(blog.created_at).toLocaleDateString(dateLocale, { dateStyle: "long" });
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 sm:p-8 lg:p-10 xl:p-12">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-6">
            <span
              ...
              title={T("blog.wordsCountTitle").replace("{words}", String(words))}
            >
              <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              {T("blog.readingTime").replace("{minutes}", String(minutes))}
            </span>
            <span className="text-gray-400 dark:text-gray-500 text-xs">{T("blog.wordsCount").replace("{words}", String(words))}</span>
          ...
          <div className="flex items-center gap-2 mb-6 pb-6 border-b border-gray-100 dark:border-gray-600">
            <span className="text-xs text-gray-500 dark:text-gray-400 font-medium">{T("blog.share")}</span>
            <a
          ...
          <BlogComments blogSlug={blog.slug} locale={locale} />
        </div>
