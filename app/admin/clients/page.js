  ...
  return (
    <div className="w-full max-w-5xl xl:max-w-none xl:px-8 2xl:px-12 mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Mutlu Müşteriler</h1>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-500">Henüz müşteri logosu yok. Yeni logo ekleyin.</div>
        ) : (
          <>
            {/* Mobil: kart listesi */}
              ...
              {clients.map((row) => (
                <div key={row.id} className="p-4 flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-gray-500 text-xs font-mono truncate">{row.image}</div>
                    <div className="text-gray-400 text-xs mt-0.5">Sıra: {row.sort_order}</div>
                  </div>
                ...
                <tbody>
                  {clients.map((row) => (
                    <tr key={row.id} className="border-b border-gray-100 last:border-0">
                      ...
                      <td className="p-3 text-gray-600 font-mono text-xs">{row.image}</td>
                      <td className="p-3 text-gray-500">{row.sort_order}</td>
